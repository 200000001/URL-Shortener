...
const shortenerResponse = (isValidUrl, serverMessage) => {
    ...
    if (isValidUrl) {
        ...
        message = `
            <strong>당신의 URL:</strong> 
            <a id="shorted-url" href="${serverMessage}" target="_blank">${serverMessage}</a>
            <button class="btn btn-sm btn-primary right" id="btn-copy-link">복사</button>
            <span class="mr-2 right d-none" id="copied">복사 완료</span>
...
btnShort.addEventListener('click', async () => {
    ...
    if(isValidUrl) {
        ...
        shortenerResponse(isValidUrl, '올바른 URL을 입력해주세요.')
    }    
})
